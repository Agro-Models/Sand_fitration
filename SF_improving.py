...
class CamposSSFModel:
    ...
    def update_filtration_and_headloss(self, Cp_inlet: float, u_dummy: float):
        ...
        This implementation avoids spurious nz/ΔL dependence by:
        - solving Eq. (16) analytically using the *local* β in each cell, and
        - updating α with Eq. (17) locally (per depth cell) without any /nz or *dL hacks.
        """
        ...
        # --- (a) Clean-bed porosity per cell: sand vs schmutzdecke (ε0) ---
        eps0_vec = np.full(nz, self.par.eps0, dtype=float)
        if self.n_schm_cells > 0:
            eps0_vec[: self.n_schm_cells] = float(self.eps_s)

        # Store β from previous step for deposition (explicit in time)
        beta_old = self.beta.copy()

        # --- (b) Eq. (16): solids profile C_p(L) using current β (cell-by-cell analytic) ---
        self.Cp[0] = float(Cp_inlet)
        for i in range(1, nz):
            self.Cp[i] = self.Cp[i - 1] * np.exp(-beta_old[i - 1] * dL)

        # --- (c) Eq. (17): local deposition ∂α/∂t = u β C_p with u = q/ε ---
        # Use TOTAL deposit for porosity (if you have inert+bio split), otherwise fall back to sigma_a.
        if hasattr(self, "sigma_a_inert") and hasattr(self, "sigma_a_bio"):
            sigma_a_inert = self.sigma_a_inert
            sigma_a_bio = self.sigma_a_bio
            sigma_a_total_old = sigma_a_inert + sigma_a_bio
        else:
            sigma_a_inert = self.sigma_a  # treat as "the" alpha field
            sigma_a_bio = None
            sigma_a_total_old = sigma_a_inert

        theta_old = self.par.b_bulk * sigma_a_total_old  # Eq. (19)
        theta_old = np.clip(theta_old, 0.0, 0.95 * eps0_vec)
        eps_old = eps0_vec - theta_old
        eps_old = np.maximum(eps_old, 1e-6)

        # Local pore velocity u = q/ε
        # (use eps_old which already includes current clogging via θ)
        u_vec = self.const.q / eps_old

        # Local forward-Euler update of *inert* α using Eq. (17)
        for i in range(nz):
            dalpha_dt = u_vec[i] * beta_old[i] * self.Cp[i]
            sigma_a_inert[i] += dalpha_dt * self.dt
            if sigma_a_inert[i] < 0.0:
                sigma_a_inert[i] = 0.0

        # Write back if split fields exist
        if hasattr(self, "sigma_a_inert") and hasattr(self, "sigma_a_bio"):
            self.sigma_a_inert = sigma_a_inert
            # bio stays whatever your biology module sets it to
            sigma_a_total = self.sigma_a_inert + self.sigma_a_bio
        else:
            self.sigma_a = sigma_a_inert
            sigma_a_total = self.sigma_a

        # --- (d) Eq. (19): θ = b α (use TOTAL α for hydraulics & β(θ)) ---
        theta = self.par.b_bulk * sigma_a_total
        self.sigma = theta  # store θ field

        eps_eff = eps0_vec - theta
        eps_eff = np.maximum(eps_eff, 1e-6)

        # --- (e) Eq. (18): filtration coefficient β(θ) ---
        beta0_vec = np.full(nz, self.par.beta0_sand, dtype=float)
        if self.n_schm_cells > 0:
            beta0_vec[: self.n_schm_cells] = float(self.par.beta0_schmutz)

        denom = eps0_vec - theta + 1e-10
        self.beta = beta0_vec + self.par.a1 * theta - self.par.a2 * (theta ** 2) / denom

        # --- (f) Eq. (22): headloss integration with clean-bed gradient anchor ---
        sand_start = int(self.n_schm_cells)

        # Campos Eq. (22) factor (computed across full sand-depth grid)
        # f_sand_full = (
        #     (1.0 + theta / (1.0 - self.par.eps0 + 1e-10)) ** self.par.c1
        #     * (self.par.eps0 / (self.par.eps0 - theta + 1e-10)) ** self.par.c2
        # )

        # --- (f) Eq. (22): headloss factor, made numerically + physically safe ---

        # 1) enforce physical constraint: theta must stay strictly below eps0 in sand
        theta_safe = np.array(theta, dtype=float, copy=True)

        # only apply this constraint to sand porosity reference (eps0), since Eq. (22) uses eps0
        # keep a small margin so (eps0 - theta) never hits 0
        theta_safe = np.clip(theta_safe, 0.0, 0.95 * self.par.eps0)

        # 2) build the two factors with a strictly-positive denominator
        den_sand = (self.par.eps0 - theta_safe)
        den_sand = np.maximum(den_sand, 1e-10)  # guarantees positive base

        term1 = 1.0 + theta_safe / (1.0 - self.par.eps0 + 1e-10)
        term2 = self.par.eps0 / den_sand

        # 3) Eq. (22) factor over full sand-depth grid
        f_sand_full = (term1 ** self.par.c1) * (term2 ** self.par.c2)
        ...
        K0 = float(self.K0)     # clean-bed hydraulic gradient (dH/dL)_0  [m/m]
        H0 = float(self.H_tot0) # clean-bed total headloss [m]

        # Sand contribution integrated over [0, L0] using ΔL
        self.dH_dL[:] = K0 * f_sand_full
        self.dH_cell[:] = self.dH_dL[:] * dL

        # Schmutzdecke as an ADDED slab headloss of thickness dLs (nz-invariant)
        H_schm = 0.0
        if (not self.is_covered) and (sand_start > 0) and (self.dLs > 0.0):
            schm_ratio = float(self.compute_happel_schmutzdecke())
            K_schm = K0 * schm_ratio
            H_schm = float(K_schm * self.dLs)

        # Totals
        self.H_cum[:] = np.cumsum(self.dH_cell)
        H_sand = float(np.sum(self.dH_cell))
        self.H_total_m = float(H_sand + H_schm)
        self.H_total_ratio = float(self.H_total_m / (H0 + 1e-30))

        # Optional diagnostic: Eq. (22) factor “per layer” for plotting
