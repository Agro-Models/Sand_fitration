...
#
# Schmutzdecke (surface layer treated as lumped):
...
class SSFInfluent:
    ...
    def from_excel(cls, path: str, sheet: str = "SSFInfluent") -> "SSFInfluent":
        ...
        return cls(
            ...
            D=float(values["D"]),
            z=float(values["z"]),
            Cp_inlet=float(values["Cp_inlet"]),
            n_tot=float(values["n_tot"]),
        )
...
class CamposSSFModel:
    ...
    def __post_init__(self):
        ...
        # Supernatant (you can later move these to Excel if you like)
    ...
    def algal_growth_rate_sup(self, n_tot: float, ps: float, T: float,
    ...
    def update_supernatant(self, T: float, Nv: float, D: float,
        ...
        """
        kg = self.algal_growth_rate_sup(n_tot, self.ps_sup, T, Nv, D, z)
    ...
    def update_schmutzdecke_daily(self, dt_hours: float):
        ...
        Eqs. (11–13): describe thickness and porosity update as a % growth
                      proportional to daily biomass increase. We approximate
                      this by:

            φ  = 100 * (SB_new − SB_old) / SB_old   (if SB_old > 0)
            ε_s^{new} = ε_s^{old} + (φ/100)(ε_s^{old} − 1)

        The main effect is decreasing porosity as SB increases.
        """
        if dt_hours <= 0.0:
            return
        ...
        # SBc – Eq. text: SBc = A_ca * SB  [mg C m^-2]
        self.SBc = self.SB * self.const.Aca

        if SB_prev > 0.0:
            phi = 100.0 * dSB / SB_prev
        else:
            phi = 0.0

        # porosity update in the spirit of Eqs. (11–13)
        self.eps_s = self.eps_s + (phi / 100.0) * (self.eps_s - 1.0)

        # Eq. (10) for thickness (using initial ε0s)
    ...
    def update_filtration_and_headloss(self, Cp_inlet: float, u: float):
        ...
        eps0 = self.par.eps0
        self.beta = (self.par.beta0_sand
                     + self.par.a1 * self.sigma
                     - self.par.a2 * (self.sigma ** 2) / (eps0 - self.sigma + 1e-30))
    ...
    def update_biology(self, T: float):
        ...
        Top 2 cm (here approximated as first layer j=0) splits carbon inputs
        2/41 from DOC, 38/41 from SB; deeper layers 40/41 from DOC, none from SB.
        ...
        for j in range(self.nz):
            ...
            # ---- Eq. (28): DOC ----
            factor_b = 2.0 / 41.0 if is_top_layer else 40.0 / 41.0
            dcd_dt = (kp * cp
                      - kh * cd
    ...
    def step(self, T: float, Nv: float, D: float, z: float,
             Cp_inlet: float, n_tot: float):
        """
        Apply one global Δt step to all coupled components:

        1) Supernatant (Eqs. 1–8)
        2) Schmutzdecke (Eqs. 9–15)
        3) Filtration & headloss (Eqs. 16–22)
        4) In-bed biology (Eqs. 23–29)
        """
        self.update_supernatant(T=T, Nv=Nv, D=D, z=z, n_tot=n_tot)
...
if __name__ == "__main__":
    ...
    # --- Time loop with history collection (THIS is what feeds df_results) ---
    history = []

    for istep in range(cfg.n_steps):
        t = istep * cfg.dt  # time in hours
        ...
        model.step(
            ...
            D=infl.D,
            z=infl.z,
            Cp_inlet=infl.Cp_inlet,
            n_tot=infl.n_tot,
        )

        # append whatever you want to analyse later
        history.append({
            "time_h": t,
            "a_sup": model.a_sup,
            "ps_sup": model.ps_sup,
    ...
    # --- Quick terminal summary (optional) ---
    print("Supernatant states after", cfg.n_steps, "h")
    print("  a_sup  [mg Chla/L]:", model.a_sup)
    print("  ps_sup [mg P/L]   :", model.ps_sup)
    print("  na_sup [mg N/L]   :", model.na_sup)
    print("  ni_sup [mg N/L]   :", model.ni_sup)

    # --- Build DataFrame + CSV + expose global for IDE ---
